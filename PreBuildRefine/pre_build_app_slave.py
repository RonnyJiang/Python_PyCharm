...
"""
 ...
 @site: www.lemon.pub
 @software: PyCharm  @since:python 3.5.2(32bit) on 2016/11/14.13:39
"""
...
PRE_BUILD_EMAIL = 'pre_build@china-liantong.com'
PRE_BUILD_PSSWORD = 'Aa123456'
LT_SMTP_SERVER = 'smtp.china-liantong.com'
CCACHE_FOLDER_NAME = ".ccache"
GERRIT_URL = "http://gerrit.china-liantong.com:2089/"
MAIL_CC = ["CarlTao@china-liantong.com", "ShengZhang@china-liantong.com", "ShaohuaAi@china-liantong.com"]
...
def reset_code(path, project_name):
    ...
    if status == 0:
        for project_path in project_paths:
            ...
            # LOG.info("%s"%change)
            if change.find("Untracked files:") >= 0 or change.find(
                    'use "git push" to publish your local commits') >= 0 or change.find("nothing to commit") == -1:
...
def post_build(base_path):
    ...
    ref_spec_new = ','.join(ref_spec.split('/')[-2:])
